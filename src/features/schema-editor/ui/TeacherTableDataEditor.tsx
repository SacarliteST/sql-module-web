...
import type { ProblemDetails, TableCellRequest, TableRowChange, TableRowResponse } from '../../../api/sqlmodule/model';
...
export function TeacherTableDataEditor({ targetDbId }: { targetDbId: string }) {
  ...
  return <Stack gap="md">
    ...
    {!tableId ? <AppCard><EmptyState title="Нет таблиц" description="Сначала создайте и примените схему базы." /></AppCard> : rowsQuery.isPending ? <AppCard><Text c="dimmed">Загрузка строк...</Text></AppCard> : !pageData ? <Alert color="red">Не удалось загрузить данные таблицы.</Alert> : <AppCard><Stack><Group justify="space-between"><Title order={4}>Строки таблицы</Title><Text size="sm" c="dimmed">Всего: {pageData.count}</Text></Group><Table.ScrollContainer minWidth={Math.max(700, columns.length * 220)}><Table withTableBorder withColumnBorders highlightOnHover><Table.Thead><Table.Tr>{columns.map((column) => <Table.Th key={column.id}>{column.name}<Text size="xs" c="dimmed">{column.physicalTypeName}{column.isRequired ? ' · NOT NULL' : ''}</Text></Table.Th>)}<Table.Th w={100} /></Table.Tr></Table.Thead><Table.Tbody>{rows.map((row, rowIndex) => <Table.Tr key={row.id ?? row.tempId}>{columns.map((column) => { const cell = row.cells[column.id] ?? { value: null, isNull: true }; return <Table.Td key={column.id}><Stack gap={4}><TextInput disabled={!canEdit || Boolean(cell.isNull)} value={cell.value ?? ''} placeholder={cell.isNull ? 'NULL' : ''} onChange={(event) => updateCell(rowIndex, column.id, { value: event.currentTarget.value, isNull: false })} /><Checkbox size="xs" disabled={!canEdit || column.isRequired} checked={Boolean(cell.isNull)} label="NULL" onChange={(event) => updateCell(rowIndex, column.id, { value: event.currentTarget.checked ? null : '', isNull: event.currentTarget.checked })} /></Stack></Table.Td>; })}<Table.Td><Button size="xs" color="red" variant="subtle" disabled={!canEdit} onClick={() => removeRow(rowIndex)}>Удалить</Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></Table.ScrollContainer>{!rows.length ? <EmptyState title="Строк нет" description="Добавьте первую учебную строку." /> : null}{pageCount > 1 ? <Pagination value={page} total={pageCount} onChange={(value) => { if (hasChanges && !confirm('Несохранённые изменения будут потеряны. Продолжить?')) return; setPage(value); }} /> : null}</Stack></AppCard>}
  </Stack>;
}
